...
#include "I2C.h"
#include "mpu6050.h"
#include "../include/rtc/rtcaccess.h"
#include "../../sdk/esp_iot_sdk_v2.2.1/include/osapi.h"
#include "../../sdk/esp_iot_sdk_v2.2.1/include/user_interface.h"
...
get_ms(unsigned long *count) {
    *count = rtc_reg_read(RTC_COUNTER_ADDR);
}
void delay_ms(unsigned long num_ms) {
    os_delay_us((uint32_t)num_ms);
...
#if defined MPU6050
const struct gyro_reg_s reg = { .who_am_i = 0x75, .rate_div = 0x19, .lpf = 0x1A,
.prod_id = 0x0C, .user_ctrl = 0x6A, .fifo_en = 0x23, .gyro_cfg = 0x1B,
.accel_cfg = 0x1C, .motion_thr = 0x1F, .motion_dur = 0x20, .fifo_count_h = 0x72,
.fifo_r_w = 0x74, .raw_gyro = 0x43, .raw_accel = 0x3B, .temp = 0x41,
.int_enable = 0x38, .dmp_int_status = 0x39, .int_status = 0x3A, .pwr_mgmt_1 =
0x6B, .pwr_mgmt_2 = 0x6C, .int_pin_cfg = 0x37, .mem_r_w = 0x6F, .accel_offs =
0x06, .i2c_mst = 0x24, .bank_sel = 0x6D, .mem_start_addr = 0x6E, .prgm_start_h =
0x70
#ifdef AK89xx_SECONDARY
,.raw_compass = 0x49,
.yg_offs_tc = 0x01,
...
const struct hw_s hw = { .addr = devAddr, .max_fifo = 1024, .num_reg = 118,
        .temp_sens = 340, .temp_offset = -521, .bank_size = 256
#if defined AK89xx_SECONDARY
        ,.compass_fsr = AK89xx_FSR
#endif
};

const struct test_s test = { .gyro_sens = 32768 / 250, .accel_sens = 32768 / 16,
        .reg_rate_div = 0, /* 1kHz. */
        .reg_lpf = 1, /* 188Hz. */
        .reg_gyro_fsr = 0, /* 250dps. */
        .reg_accel_fsr = 0x18, /* 16g. */
        .wait_ms = 50, .packet_thresh = 5, /* 5% */
        .min_dps = 10.f, .max_dps = 105.f, .max_gyro_var = 0.14f, .min_g = 0.3f,
        .max_g = 0.95f, .max_accel_var = 0.14f };
...
int mpu_read_reg(unsigned char reg, unsigned char *data) {
    ...
    if (reg >= st.hw->num_reg)
        return -1;
    i2c_read(st.hw->addr, reg, 1, data);
    return data[0];
}
...
int mpu_init(void) {
    ...
    //    return -1;
    i2c_read(st.hw->addr, st.reg->accel_offs, 6, data);
    ...
    if (rev) {
        ...
        if (rev == 1)
            ...
            MPU6050_ERR("Unsupported software product rev %d.\n", rev);
            return -1;
        }
    } else {
        //if (i2c_read(st.hw->addr, st.reg->prod_id, 1, data))
        //    return -1;
        i2c_read(st.hw->addr, st.reg->prod_id, 1, data);
        rev = data[0] & 0x0F;
        if (!rev) {
            MPU6050_ERR("Product ID read as 0 indicates device is either " "incompatible or an MPU3050.\n");
            return -1;
        } else if (rev == 4) {
            MPU6050_DEBUG("Half sensitivity part found.\n");
            st.chip_cfg.accel_half = 1;
        } else
            st.chip_cfg.accel_half = 0;
    }
...
int mpu_get_gyro_reg(short *data, unsigned long *timestamp) {
    ...
    //    return -1;
    i2c_read(st.hw->addr, st.reg->raw_gyro, 6, tmp);
...
int mpu_get_accel_reg(short *data, unsigned long *timestamp) {
    ...
    //    return -1;
    i2c_read(st.hw->addr, st.reg->raw_accel, 6, tmp);
    data[0] = (tmp[0] << 8) | tmp[1];
    data[1] = (tmp[2] << 8) | tmp[3];
    data[2] = (tmp[4] << 8) | tmp[5];
    if (timestamp)
...
int mpu_get_temperature(long *data, unsigned long *timestamp) {
    ...
    //    return -1;
    i2c_read(st.hw->addr, st.reg->temp, 2, tmp);
    raw = (tmp[0] << 8) | tmp[1];
    if (timestamp) get_ms(timestamp);

    *data = (long) ((35 + ((raw - (float) st.hw->temp_offset)
           / st.hw->temp_sens)) * 65536L);
    return 0;
}
...
int mpu_set_accel_bias(const long *accel_bias) {
    ...
    //    return -1;
    i2c_read(st.hw->addr, 0x06, 6, data);
...
int mpu_read_fifo(short *gyro, short *accel, unsigned long *timestamp,
    ...
    if (fifo_count < packet_size)
        return 0;
//    MPU6050_DEBUG("FIFO count: %hd\n", fifo_count);
    if (fifo_count > (st.hw->max_fifo >> 1)) {
        /* FIFO is 50% full, better check overflow bit. */
        //if (i2c_read(st.hw->addr, st.reg->int_status, 1, data))
        //    return -1;
        i2c_read(st.hw->addr, st.reg->int_status, 1, data);
    ...
    //    return -1;
    i2c_read(st.hw->addr, st.reg->fifo_r_w, packet_size, data);
...
int mpu_read_fifo_stream(unsigned short length, unsigned char *data,
    ...
    //    return -1;
    i2c_read(st.hw->addr, st.reg->fifo_r_w, length, data);
...
static int get_accel_prod_shift(float *st_shift) {
    ...
    //    return 0x07;
    i2c_read(st.hw->addr, 0x0D, 4, tmp);
...
static int gyro_self_test(long *bias_regular, long *bias_st) {
    ...
    //    return 0x07;
    i2c_read(st.hw->addr, 0x0D, 3, tmp);
...
#else
    gyro[0] = (long) (((long long) gyro[0] << 16) / test.gyro_sens
            / packet_count);
    gyro[1] = (long) (((long long) gyro[1] << 16) / test.gyro_sens
            / packet_count);
    gyro[2] = (long) (((long long) gyro[2] << 16) / test.gyro_sens
            / packet_count);
...
int mpu_write_mem(unsigned short mem_addr, unsigned short length,
        unsigned char *data) {
    unsigned char tmp[2];
    MPU6050_DEBUG("mpu_write_mem: mem_addr(%04x), length(%d)\n",mem_addr, length);
    if (!data)
        return -1;
    if (!st.chip_cfg.sensors)
        return -1;
...
int mpu_read_mem(unsigned short mem_addr, unsigned short length,
    ...
    //    return -1;
    i2c_read(st.hw->addr, st.reg->mem_r_w, length, data);
    return 0;
}
