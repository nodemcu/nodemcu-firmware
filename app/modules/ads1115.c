...
static uint16_t ads1115_threshold_low = 0x3FFF;		// 16383	1/4
static uint16_t ads1115_threshold_hi = 0xBFFD;		// 49149	3/4
...
static uint8_t get_value(int16_t *volt) {
	...
	switch (ads1115_gain) {
		case (ADS1115_PGA_6_144V):
			if ((*volt >= 6144) || (*volt < -6144) || ((*volt < 0) && (ads1115_mode >> 2))) return 1;
			*volt = *volt / 0.1875;
			break;
		case (ADS1115_PGA_4_096V):
			if ((*volt >= 4096) || (*volt < -4096) || ((*volt < 0) && (ads1115_mode >> 2))) return 1;
			*volt = *volt / 0.125;
			break;
		case (ADS1115_PGA_2_048V):
			if ((*volt >= 2048) || (*volt < -2048) || ((*volt < 0) && (ads1115_mode >> 2))) return 1;
			*volt = *volt / 0.0625;
			break;
		case (ADS1115_PGA_1_024V):
			if ((*volt >= 1024) || (*volt < -1024) || ((*volt < 0) && (ads1115_mode >> 2))) return 1;
			*volt = *volt / 0.03125;
			break;
		case (ADS1115_PGA_0_512V):
			if ((*volt >= 512) || (*volt < -512) || ((*volt < 0) && (ads1115_mode >> 2))) return 1;
			*volt = *volt / 0.015625;
			break;
		case (ADS1115_PGA_0_256V):
			if ((*volt >= 256) || (*volt < -256) || ((*volt < 0) && (ads1115_mode >> 2))) return 1;
			*volt = *volt / 0.0078125;
			break;
	}
...
// wait for ADC conversion corresponding to data rate settings
static uint16_t conversion_delay(void) {
...
// Change ADC settings
// Lua: 	ads1115.setting(GAIN,SAMPLES,CHANNEL[,COMPARATOR,THRESHOLD_LOW,THRESHOLD_HI])
static int ads1115_lua_setting(lua_State* L) {
	...
	ads1115_mode = ADS1115_MODE_SINGLE;
	...
	if (lua_isnumber(L, 4)) {
		...
		ads1115_comp = luaL_checkinteger(L, 4);
		...
		ads1115_mode = ADS1115_MODE_CONTIN;
