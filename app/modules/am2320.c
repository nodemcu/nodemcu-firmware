...
static uint16_t ICACHE_FLASH_ATTR crc16(uint8_t *ptr, unsigned int len)
{
    uint16_t	crc =0xFFFF;
    uint8_t	i;
...
static int ICACHE_FLASH_ATTR _read(uint32_t id, void *buf, uint8_t len, uint8_t off)
{
    int i;
    ...
    crc = platform_i2c_recv_byte(id,1) | (platform_i2c_recv_byte(id,0) << 8);
    platform_i2c_send_stop(id);
...
static int ICACHE_FLASH_ATTR am2320_init(lua_State* L)
{
    uint32_t sda;
    uint32_t scl;
    int ret;
...
static int ICACHE_FLASH_ATTR am2320_read(lua_State* L)
{
    int ret;
    struct {
    	uint8_t  cmd;
