...
#define BME280_SAMPLING_DELAY  113000 //maximum measurement time for maximum oversampling for all measures = 1.25 + 2.3*16 + 2.3*16 + 0.575 + 2.3*16 + 0.575 ms
...
static uint8_t ICACHE_FLASH_ATTR r8u(uint8_t reg) {
	uint8_t ret;
...
static uint8_t ICACHE_FLASH_ATTR w8u(uint8_t reg, uint8_t val) {
	platform_i2c_send_start(bme280_i2c_id);
	platform_i2c_send_address(bme280_i2c_id, bme280_i2c_addr, PLATFORM_I2C_DIRECTION_TRANSMITTER);
	platform_i2c_send_byte(bme280_i2c_id, reg);
...
static uint16_t ICACHE_FLASH_ATTR r16u(uint8_t reg) {
	uint8_t high = r8u(reg);
	uint8_t low  = r8u(++reg);
	return (high << 8) | low;
}

static uint16_t ICACHE_FLASH_ATTR r16uLE(uint8_t reg) {
	uint8_t low = r8u(reg);
	uint8_t high  = r8u(++reg);
	return (high << 8) | low;
}

static uint32_t ICACHE_FLASH_ATTR r24u(uint8_t reg) {
	uint8_t high = r8u(reg);
	uint8_t mid  = r8u(++reg);
	uint8_t low  = r8u(++reg);
...
static BME280_S32_t ICACHE_FLASH_ATTR bme280_compensate_T(BME280_S32_t adc_T) {
...
static BME280_U32_t ICACHE_FLASH_ATTR bme280_compensate_P(BME280_S32_t adc_P) {
...
static BME280_U32_t ICACHE_FLASH_ATTR bme280_compensate_H(BME280_S32_t adc_H) {
...
static int ICACHE_FLASH_ATTR bme280_lua_init(lua_State* L) {
	uint8_t sda;
	uint8_t scl;
	uint8_t ossh;
	uint8_t config;
	...
	platform_i2c_send_start(bme280_i2c_id);
	uint8_t ack = platform_i2c_send_address(bme280_i2c_id, bme280_i2c_addr, PLATFORM_I2C_DIRECTION_TRANSMITTER);
	platform_i2c_send_stop(bme280_i2c_id);
	if (!ack) {
...
static int ICACHE_FLASH_ATTR bme280_lua_temp(lua_State* L) {
	if (lua_isnumber(L, 1)) { // forced mode
		int32_t delay = luaL_checkinteger(L, 1);
		if (delay==0) {delay=BME280_SAMPLING_DELAY;}
...
static int ICACHE_FLASH_ATTR bme280_lua_baro(lua_State* L) {
	if (lua_isnumber(L, 1)) { // forced mode
		int32_t delay = luaL_checkinteger(L, 1);
		if (delay==0) {delay=BME280_SAMPLING_DELAY;}
		w8u(BME280_REGISTER_CONTROL, (bme280_mode & 0xFC) | BME280_FORCED_MODE);
...
static int ICACHE_FLASH_ATTR bme280_lua_humi(lua_State* L) {
...
static int ICACHE_FLASH_ATTR bme280_lua_qfe2qnh(lua_State* L) {
	if (!lua_isnumber(L, 2)) {
		return luaL_error(L, "wrong arg range");
	}
	...
	double qnh = (double)qfe * hc + 0.0005;
	lua_pushinteger(L, (int32_t)qnh);
	return 1;
}

static int ICACHE_FLASH_ATTR bme280_lua_altitude(lua_State* L) {
	if (!lua_isnumber(L, 2)) {
		return luaL_error(L, "wrong arg range");
	}
	...
	double h = (pow((double)P/(double)qnh, 1.0/-5.25588) - 1.0) / 2.25577e-5 * 100.0 + 0.005;
	lua_pushinteger(L, (int32_t)h);
	return 1;
}

static int ICACHE_FLASH_ATTR bme280_lua_dewpoint(lua_State* L) {
	...
	int32_t T = luaL_checkinteger(L, 2);

	double ln = log(H/100000.0 * exp((c17 * T/100.0) / (c243 + T/100.0)));
	double d = (243.5 * ln)/(c17 - ln) * 100.0 + 0.005;
	lua_pushinteger(L, (int32_t)d);
	return 1;
}
