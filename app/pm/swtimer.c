...
#ifdef LUA_USE_MODULES_TIMER_SUSPEND_DEBUG
#define SWTMR_DEBUG
#endif
...
#define CB_LIST_STR "cb_list"
#define SUSP_LIST_STR "susp_list"
...
void swtmr_cb_register(void* timer_cb_ptr);
static void add_to_reg_queue(void* timer_cb_ptr);
...
void swtmr_suspend_timers(){
  ...
  os_timer_func_t** tmr_cb_ptr_array = c_zalloc(sizeof(os_timer_func_t*)*registered_cb_qty);
  if(!tmr_cb_ptr_array){
  ...
  while(lua_next(L, -2) != 0){
    if(lua_isuserdata(L, -1)){
      tmr_cb_ptr_array[index] = lua_touserdata(L, -1);
    }
    lua_pop(L, 1);
    index++;
  }
  ...
  while(timer_ptr != NULL){
    ...
    for(int i = 0; i < registered_cb_qty; i++){
      if(timer_ptr->timer_func == tmr_cb_ptr_array[i]){
    ...
    if(next_timer != (os_timer_t*)0xffffffff){
      SWTMR_DBG("timer(%p) was suspended", timer_ptr);
...
void swtmr_cb_register(void* timer_cb_ptr){
  ...
  if(timer_cb_ptr){
    ...
    if(lua_istable(L, -1)){
      ...
      //check if timer cb is already registered
      if(cb_list_last_idx != 0){
        lua_pushnil(L);
        while(lua_next(L, -2) != 0){
          if(lua_isuserdata(L, -1) && lua_touserdata(L, -1) == timer_cb_ptr){
            //timer is already registered, return
            return;
          }
          lua_pop(L, 1);
        }
    ...
    lua_pushlightuserdata(L, timer_cb_ptr);
    lua_rawset(L, -3);
...
int print_timer_list(lua_State* L){
  os_timer_t* timer_list_ptr = timer_list;
  c_printf("Current FRC2: %u\n", RTC_REG_READ(FRC2_COUNT_ADDRESS));
  c_printf("timer_list:\n");
  while(timer_list_ptr != NULL){
    c_printf("ptr:%p\tcb:%p\texpire:%8u\tperiod:%8u\tnext:%p\n",timer_list_ptr, timer_list_ptr->timer_func, timer_list_ptr->timer_expire, timer_list_ptr->timer_period, timer_list_ptr->timer_next);
    timer_list_ptr = timer_list_ptr->timer_next;
  }
  return 0;
}
