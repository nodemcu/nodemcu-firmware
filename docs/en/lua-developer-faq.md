...
-  Johny Mattsson's software exception handler and my LCD patch have allowed us to move a lot of constant data out of RAM and into the firmware address space, and as a result current builds now typically boot with over 40Kb free RAM instead of 15Kb free and the code density is roughly 40% better.
-  I fixed error reporting so errors now correctly report line numbers in tracebacks.
...
*  All other service and application processing is split into code execution blocks, known as **tasks**.  The individual are executed one at a time and run to completion. No task can never pre-empt another.
...
* Excessively long-running Lua functions can cause other system functions and services to timeout, or to allocate scarce RAM resources to buffer queued data, which can then trigger either the watchdog timer or memory exhaustion, both of which will ultimately cause the system to reboot.
...
```Lua 
  do
    ...
    tmr2:alarm(300000, tmr.ALARM_AUTO, tmr2unc)
  end
  -- 
```
