...
The broker will publish a clients last will message once he NOTICES that the connection to the client is broken.  
The broker will notice this when:
...
This means if you specified 120 as keepalive timer, just turn of the client device and the broker does not send any data to the client, the last will message will be published 120s after turning off the device.
