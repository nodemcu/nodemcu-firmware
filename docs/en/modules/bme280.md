...
####Returns
`nil` initialization has failed (no sensor connected?), `true` sensor is BME280, `false` sensor is BMP280
...
####Parameters  
* (optional) `delay` - sets sensor to forced mode, and waits given number of microseconds before reading the sensor. If 0 then default delay is set to 113ms (sufficient time to perform reading for oversampling settings 16x). Leave empty when operating in sensor normal mode.
...
###baro()
####Description
Reads the sensor and returns the air temperature in hectopascals as an integer multiplied with 1000.
