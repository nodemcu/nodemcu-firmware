...
local function initBuffer(b, ...)
  for i,v in ipairs({...}) do
    b:set(i, v, v*2, v*3, v*4)
  end
  return b
end

N.test('initialize a buffer', function()
    local buffer = pixbuf.newBuffer(9, pixbuf.TYPE_GRB)
    nok(buffer == nil)
    ...
    fail(function() pixbuf.newBuffer(9, -1) end, "must be a valid pixbuf type")
    fail(function() pixbuf.newBuffer(0, pixbuf.TYPE_GRB) end, "should be a positive integer")
    fail(function() pixbuf.newBuffer(-1, pixbuf.TYPE_GRB) end, "should be a positive integer")
end)
...
N.test('have correct size', function()
    ...
    ok(eq(buffer:size(), 9), "check size")
    buffer = pixbuf.newBuffer(13, pixbuf.TYPE_I5BGR)
    ok(eq(buffer:size(), 13), "check size")
end)

N.test('fill a buffer with one color', function()
    local buffer = pixbuf.newBuffer(3, pixbuf.TYPE_GRB)
...
N.test('get/set correctly', function()
    local buffer = pixbuf.newBuffer(3, pixbuf.TYPE_GRBW)
...
N.test('get/set multiple with string', function()
    ...
    local buffer = pixbuf.newBuffer(4, pixbuf.TYPE_GRB):set(1,"ABCDEF")
    buffer:setRGB(3,"LMNOPQ")
    ok(eq(buffer:dump(), "ABCDEFMLNPOQ"))
...
N.test('get/set/fill WWA', function()
    local buffer = pixbuf.newBuffer(3, pixbuf.TYPE_WWA)
    buffer:fill(1,2,3)
    buffer:set(1,4,5,6)
    ok(eq(buffer:dump(), string.char(4,5,6,1,2,3,1,2,3)))

    fail(function() buffer:fillRGB(1,2,3) end, "Can't fill strip with RGB")
    fail(function() buffer:getRGB(1) end, "Can't convert WWA to RGB")
    fail(function() buffer:setRGB(1,1,2,3) end, "Can't setRGB on WWA pixbuf")
end)

N.test('fade correctly', function()
    local buffer = pixbuf.newBuffer(1, pixbuf.TYPE_RGB)
    buffer:fill(1,222,55)
    buffer:fade(2)
...
N.test('mix correctly issue #1736', function()
    local buffer1 = pixbuf.newBuffer(1, pixbuf.TYPE_GRB)
    local buffer2 = pixbuf.newBuffer(1, pixbuf.TYPE_GRB)
...
N.test('power', function()
    local buffer = pixbuf.newBuffer(2, pixbuf.TYPE_GRBW)
    buffer:fill(10,22,54,234)
    ok(eq(buffer:power(), 2*(10+22+54+234)))
end)

N.test('shift LOGICAL', function()
    local buffer1 = pixbuf.newBuffer(4, pixbuf.TYPE_RGBW)
    local buffer2 = pixbuf.newBuffer(4, pixbuf.TYPE_RGBW)
